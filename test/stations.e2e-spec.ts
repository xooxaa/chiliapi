...
describe('Stations Module', () => {
  ...
  it('returns 404 when trying to get a non-existent station', async () => {
    const nonExistentStationId = 'non-existent-id';
    const response = await request(app.getHttpServer()).get(`/stations/${nonExistentStationId}`).expect(404);
  ...
  it('retrieves all stations', async () => {
    await request(app.getHttpServer())
      .post('/stations')
      .send({ name: 'Station 2', description: 'My Station 2' })
      .expect(201);

    const response = await request(app.getHttpServer()).get('/stations').expect(200);
  ...
  it('updates a station', async () => {
    ...
    const updatedStationResponse = await request(app.getHttpServer())
  ...
  it('deletes a station', async () => {
    ...
    await request(app.getHttpServer()).delete(`/stations/${stationId}`).expect(200);
    await request(app.getHttpServer()).get(`/stations/${stationId}`).expect(404);
  });

  it('returns 404 when trying to get sensors from a non-existent station', async () => {
    const nonExistentStationId = 'non-existent-id';
    const response = await request(app.getHttpServer()).get(`/stations/${nonExistentStationId}/sensors`).expect(404);
  ...
  it('retrieves all sensors for a station', async () => {
    ...
    const sensor1Response = await request(app.getHttpServer())
      .post('/sensors')
      .send({ name: 'Sensor 1', type: 'temperature', stationId })
      .expect(201);
    const sensor2Response = await request(app.getHttpServer())
      .post('/sensors')
    ...
    const response = await request(app.getHttpServer()).get(`/stations/${stationId}/sensors`).expect(200);
